describe('Conditional Choices in Tables', () => {
    ...
    it('should have correct field names in column 1', () => {
        // Verify all fields in first column have index _1
        cy.get('select[name="products_table._1.category"]').should('exist');
        cy.get('select[name="products_table._1.product"]').should('exist');
        cy.get('input[name="products_table._1.quantity"]').should('exist');

        // Verify all fields in second column have index _2
        cy.get('select[name="products_table._2.category"]').should('exist');
        cy.get('select[name="products_table._2.product"]').should('exist');
        cy.get('input[name="products_table._2.quantity"]').should('exist');

        // Verify all fields in third column have index _3
        cy.get('select[name="products_table._3.category"]').should('exist');
        cy.get('select[name="products_table._3.product"]').should('exist');
        cy.get('input[name="products_table._3.quantity"]').should('exist');
    });

    it('should show fruit choices when category is fruits in column 1', () => {
        // Select category in first column
        cy.get('select[name="products_table._1.category"]').select('fruits');

        // Verify product dropdown shows fruit choices
        cy.get('select[name="products_table._1.product"] option').then($options => {
    ...
    it('should show vegetable choices when category is vegetables in column 1', () => {
        cy.get('select[name="products_table._1.category"]').select('vegetables');
    ...
    it('should maintain independent choices across different table columns', () => {
        ...
        cy.get('select[name="products_table._2.category"]').select('vegetables');
        cy.get('select[name="products_table._2.product"] option').then($options => {
            const values = [...$options].map(o => o.value).filter(v => v !== '');
            expect(values).to.deep.equal(['carrot', 'broccoli', 'lettuce']);
        });
    ...
    it('should update product choices when category is changed', () => {
        // Start with fruits
        cy.get('select[name="products_table._1.category"]').select('fruits');
        cy.get('select[name="products_table._1.product"]').select('apple');
        ...
        cy.get('select[name="products_table._1.product"]').should('have.value', '');
    });
    ...
    it('should test all three columns independently', () => {
        ...
        cy.get('select[name="products_table._3.category"]').select('vegetables');
        cy.wait(500);
        cy.get('select[name="products_table._3.product"] option').then($options => {
            const values = [...$options].map(o => o.value).filter(v => v !== '');
    ...
    it('should work with pre-filled values on initial render', () => {
        ...
        cy.get('select[name="products_table._1.category"]').select('fruits');
        cy.get('select[name="products_table._1.product"]').select('banana');
        cy.get('input[name="products_table._1.quantity"]').type('5');
        ...
        cy.get('select[name="products_table._1.category"]').should('have.value', 'fruits');

        // Verify product dropdown shows fruit choices and has correct selection
        cy.get('select[name="products_table._1.product"]').should('have.value', 'banana');
        cy.get('select[name="products_table._1.product"] option').then($options => {
            const values = [...$options].map(o => o.value).filter(v => v !== '');
            expect(values).to.deep.equal(['apple', 'banana', 'orange']);
        });
